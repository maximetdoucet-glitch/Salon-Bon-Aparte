(() => {
  ...
  if (bookModal) {
    ...
    function buildIcsDataUri(start, end) {
      ...
      const uid = 'ba-' + Math.random().toString(36).slice(2) + '@salonbonaparte.nl';
      const summary = `Salon Bon Aparte — ${state.serviceLabel}`;
      const description = `Afspraak: ${state.serviceLabel}\\nBarber: ${state.barberLabel}\\nNaam: ${state.name}\\nPrijs: €${state.price}\\n\\nHertogstraat 120A, 6511 SG Nijmegen`;

      const ics = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Salon Bon Aparte//Booking//NL',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        'BEGIN:VEVENT',
        `UID:${uid}`,
        ...
        'LOCATION:Hertogstraat 120A\\, 6511 SG Nijmegen',
        'END:VEVENT',
        'END:VCALENDAR'
      ].join('\r\n');
